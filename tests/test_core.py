...
def test_den2freq_array():
    ...
    assert np.allclose(result, expected, rtol=1e-8), "Incorrect frequency for
    array input"
...
def test_freq2den_scalar():
    ...
    assert isinstance(result, float), "Result should be a float"
    assert result == pytest.approx(expected, rel=1e-8), "Incorrect density for
    scalar input"
...
def test_freq2den_array():
    ...
    assert isinstance(result, np.ndarray), "Result should be a NumPy array"
    assert np.allclose(result, expected, rtol=1e-8), "Incorrect density for
    array input"


def test_freq2den_negative_result_not_expected():
    """Test that freq2den doesn't produce negative densities for valid
    input."""
...
def test_find_X_scalar():
    ...
    result = find_X(n_e, f)
    assert isinstance(result, float), "Result should be a float"
    assert result == pytest.approx(expected, rel=1e-8), "Incorrect X value for
    scalar input"
...
def test_find_X_array():
    ...
    result = find_X(n_e, f)
    assert isinstance(result, np.ndarray), "Result should be a NumPy array"
    assert np.allclose(result, expected, rtol=1e-8), "Incorrect X values for
    array input"
...
def test_find_Y_scalar():
    ...
    result = find_Y(f, b)
    assert isinstance(result, float), "Result should be a float"
    assert result == pytest.approx(expected, rel=1e-8), "Incorrect Y value for
    scalar input"
...
def test_find_Y_array():
    ...
    result = find_Y(f, b)
    assert isinstance(result, np.ndarray), "Result should be a NumPy array"
    assert np.allclose(result, expected, rtol=1e-8), "Incorrect Y values for
    array input"
...
def test_virtical_forward_operator_basic_O_mode():
    """Basic test for virtical_forward_operator in 'O' mode with short
    arrays."""
...
def test_virtical_forward_operator_O_and_X_modes():
    """Test virtical_forward_operator for both O and X modes with minimal
    inputs."""
    # Test input arrays
    freq = np.array([1.0, 2.0, 3.0])  # MHz
    alt = np.array([100, 200, 300])  # km
    ...
    for mode in ['O', 'X']:
        ...
        assert isinstance(vh, np.ndarray)
        assert vh.shape == freq.shape
        assert np.isnan(vh[-1]), f"Expected NaN for freq above foF2 in mode
        {mode}"
        assert np.all(np.isfinite(vh[:-1])), f"Expected finite heights below
        foF2 in mode {mode}"